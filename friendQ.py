...
class FriendQLearner:
    def __init__(self,
        ...
        q_table = np.full(shape=(num_states, num_actions, num_actions), fill_value=0.0)
    ...
    def update_Q(self, actions, state_prime, r, player_name):
        ...
        error = ((r + self.gamma * V) - self.q_tables[player_name][state_index, actions['A'], actions['B']]) * self.alpha
    ...
    def plot_error(self, experiment_id):
        ...
        plt.title("Friend-Q")
        plt.savefig('outputs/FriendQ_exp_' + str(experiment_id) + '_' + str(self.step_count) + '.png')
        plt.show(block=False)
        plt.show()


    def save_data(self, experiment_id):
        error_file_name = 'outputs/data_FriendQ_error_exp_' + str(experiment_id) + '.txt'
        error_file = open(error_file_name, 'w')
        for item in self.ERRs:
            error_file.write("%s\n" % item)

        # step_file = open('step_test.txt', 'w')
        step_file_name = 'outputs/data_FriendQ_step_exp_' + str(experiment_id) + '.txt'
...
def main():
    learner = FriendQLearner()
    learner.friendQ_agent()
